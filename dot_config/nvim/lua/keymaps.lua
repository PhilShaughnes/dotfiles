...
-- todo
set_group('n', '<leader>t', 'todo')
-- M.nmap('<leader>tt', todo_toggle, { desc = "toggle as todo" })
-- M.nmap('<leader>tx', todo_status, { desc = "toggle todo status" })
-- module:
-- for line or visual selection:
-- if see [ ] (with one character), change what is inside
-- if list delimiter (-,*, maybe letters or numbers?) then:
-- add [ ], or remove if present
-- module:
-- treesitter highlighting for todo lists
